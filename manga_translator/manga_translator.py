...
class MangaTranslator():
    ...
    async def _translate(self, ctx: Context) -> Context:
        ...
        if self.verbose:
            ...
            inputmask=cv2.cvtColor(inpaint_input_img, cv2.COLOR_RGB2BGR)
            cv2.imwrite(self._result_path('inpaint_input.png'), inputmask)
            cv2.imwrite(self._result_path('mask_final.png'), ctx.mask)


            newmask=os.path.join(BASE_PATH, 'tmp', self.result_sub_folder)
            newmask=os.path.join(newmask,ctx.input.name+'.mask_final.png')
            cv2.imwrite(newmask, ctx.mask)
            #ctx.mask=self._guolv(ctx.mask,inpaint_input_img)
