...
def extract_text_regions(mask_img,textlines):
    h,w=mask_img.shape
    extracted_mask=np.zeros((h,w),dtype=np.uint8)
    for(x,y,w,h) in textlines:
        mask_region=mask_img[y:y+h,x:x+w]
        extracted_mask[y:y+h,x:x+w][mask_region==255]=255
